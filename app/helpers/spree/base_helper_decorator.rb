Spree::BaseHelper.module_eval do
  def link_to_facet(facet_name, facet_row)
    ...
    if facet_name == :taxon_id
      ...
      link_to(facet_row.instance.name, collections_path(facet_row.instance.permalink, params.merge("page" => nil))) + " (#{facet_row.count})"
    elsif facet_name == :featured
      link_to("Lekarnarjev izbor", params.merge("#{facet_name}_facet" => facet_row.value, "page" => nil)) + " (#{facet_row.count})"
    else
